...
const ReactDateRange = () => {
  ...
  // input field e kon style date dekhabe tar jonno date formate kora hoiche. orthat prothome "barer nam" dekhabe tarpor "bochorer nam" dekhabe ebong last e dekhabe "mash er nam"
  const formattedStartDate = format(date[0].startDate, "EEEE, do MMMM");
  const formattedEndDate = format(date[0].endDate, "EEEE, do MMMM");
  // api ke call korar jonno date formate kora hoiche. karon api te hit korar jonno amader ei formate hite korte hobe. pothome "bochor" tarpor "-" tarpor "mash" tarpor "-"" ebong tarpor "tarikh"
  ...
  // user ekta date range select korle jeno calendar hide hoye jay tar jonno kora hoiche
  useEffect(() => {
      ...
      <div className="relative">
        ...
        >
          <div className="relative cursor-pointer">
            ...
            <input
              type="search"
              id="search"
              className="block w-full py-1 pl-8 text-gray-900 text-sm leading-4 resize-none outline-none rounded-md cursor-pointer"
              value={`${formattedStartDate}`}
              readOnly
            />
          </div>

          <label className=" text-[.75rem] text-gray-600 font-bold whitespace-nowrap cursor-pointer">
            {getTotalNights()}
          </label>
          ...
          <div className="relative">
            <input
              type="search"
              id="search"
              className="block w-full py-1 pr-8 text-gray-900 text-sm leading-4 resize-none outline-none rounded-md cursor-pointer"
        ...
        {openDate && (
          <DateRange
            ...
            className="bg-white absolute top-[97%] w-full z-50 border border-green-focus rounded-b-md shadow-lg"
          />
        )}
      </div>
    </>
